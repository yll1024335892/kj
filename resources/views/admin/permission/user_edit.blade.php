...
<article class="clearfix pd-20">
    <form action="{{route('user.update',$user->id)}}" method="post" class="form form-horizontal" id="form-admin-add">
        ...
        <div class="row clearfix">
            ...
            <div class="form-controls col-xs-8 col-sm-9">
                    ...
                    @foreach($roles as $role)
                        ...
                        @endif
                            <label class="">
                                <input type="checkbox" @if($role->id == $mark) checked @endif value="{{$role['id']}}" name="roles[]" />
                                {{$role['name']}}</label>
                    @endforeach
            </div>
        </div>
